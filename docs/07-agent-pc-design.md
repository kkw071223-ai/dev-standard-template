...
> # ⛔ 이 문서는 2026-08-13 에 퇴역했다 — §18 대로 분해되었다
>
> Phase 0–11 을 끝까지 실행했고, **§18(문서 생애주기)이 지시한 분해가 완료되었다.**
> 이 문서는 그 PC 에서 **더 이상 현재 상태가 아니다.** 여기 적힌 경로·버전·계획을
> "지금 이렇다"로 읽으면 안 된다 — §18이 경고한 바로 그 오독이다.
>
> **재구축할 때만 연다.** 그 PC 에서 일상 작업 중이라면 대신 이것들을 읽는다:
>
> | 알고 싶은 것 | 지금 읽을 곳 |
> |---|---|
> | 지금 무엇이 깔려 있나 (§2 §4 §5) | `vault/90-meta/ENVIRONMENT.md` — **자동 생성, 실측** |
> | 구조·채널 (§3 §6 §7 §8) | `vault/90-meta/architecture.md` |
> | 루프 규약·게이트·격리 (§9 §10 §12) | `C:\agent\domains\*\CLAUDE.md` ×5 — **실제로 강제되는 자리** |
> | 확인된 사실과 함정 (§1 일부, §15) | `vault/20-facts/FCT-*.md` |
> | 생태계 조사 (§1) | `vault/40-refs/REF-nvidia-stack-2026-08.md` |
> | 플로우별 실패 모드 (§11) | `vault/10-domains/*/flows/FLOW-*.md` |
> | 구축 기록 (§13 §15 §17) | `vault/90-meta/archive/2026-08-setup/` — **동결** |
>
> 구축 중 이 문서가 실제와 어긋난 지점은 아카이브 `README.md` 의 "설계서가 틀렸던 곳"
> 표에 전부 모아 두었다. 주요한 것: `wsl --install` 은 네 단계, Isaac 첫 기동은
> 10–30분이 아니라 139초, `tar -xvzf` 가 아니라 `tar -xf`, 8B VRAM 은 ~6 GB 가
> 아니라 9.0 GB.
...
| 세팅 끝난 뒤엔 뭘 보나 | [§18](#18-문서-생애주기--이-설계서는-언제-버려지나) — **이 문서는 그때 폐기된다** |
...
**중요 — 드라이버 갈래를 잘못 고르면 안 된다.** 이 GPU는 `RTX PRO 5000 Blackwell Laptop GPU`, 즉 **프로 계열**이다. GeForce Game Ready가 아니라
...
## 18. 문서 생애주기 — 이 설계서는 언제 버려지나

**이 문서는 소모품이다.** Phase 11이 끝나는 순간 §13(구축 순서)은 죽은 문서가 되고, 그 뒤로도 들고 있으면 낭비를 넘어 **위험**해진다. 에이전트가 "지난 계획"을 "현재 상태"로 착각하기 때문이다. 설계서에 `D:\agent`라고 적혀 있으면 실측이 아무리 `C:`라고 해도 언젠가는 헷갈린다.

그래서 이 설계서는 **보관되지 않는다. 분해되어 각자 다른 수명을 가진 문서로 흩어진다.**

### 18.1 문서의 세 계급

| 계급 | 예 | 수명 | 위치 |
|---|---|---|---|
| **구축 문서** | §13 Phase 0–11, §17, 사전 점검 리포트 | **세팅 완료 시 종료** | `90-meta/archive/2026-08-setup/` — 동결. 작업 컨텍스트에서 제외 |
| **상설 문서** | 도메인 계약, **플로우 문서**, 사실 카드 | 계속 갱신 | `10-domains/`, `20-facts/` |
| **사건 기록** | 런 리포트 | 불변, 누적 | `30-runs/` |
| **자동 생성** | `ENVIRONMENT.md` | 주 1회 재생성 | `90-meta/` |

### 18.2 설계서 각 절이 어디로 가나

| 이 문서의 절 | 세팅 완료 후 |
|---|---|
| §1 생태계 조사 | **사실 카드 + `40-refs/`** 로 분해. 버전이 바뀌면 만료 |
| §2 하드웨어, §4 VRAM, §5 디스크 | **`ENVIRONMENT.md`로 대체.** 계획이 아니라 실측이므로 자동 생성한다 |
| §3 구성도, §6 Obsidian, §7 Notion, §8 텔레그램 | **`90-meta/architecture.md`** — 구조가 바뀔 때만 손댄다 |
| §9 워크스페이스 스키마, §10 루프 규약, §12 게이트 | **도메인 `CLAUDE.md`로 이주.** 여기가 실제로 강제되는 자리다 |
| §11 도메인별 DAG | **플로우 문서로 분해** (§18.3) |
| §13 구축 순서, §17 사람 할 일, §15 실측 | **아카이브.** 재구축할 때만 꺼낸다 |

**Phase 11의 마지막 작업이 이 분해다.** 끝나면 이 설계서는 `90-meta/archive/`로 들어가고, 일상 작업에서 다시 열리지 않는다.

### 18.3 플로우 문서 — 상설 문서의 핵심 단위

당신이 물은 "각 프로세스나 플로우마다 MD"가 이것이다. **플로우 하나 = 문서 하나**, 그 안에 설계·구현·검증이 다 들어간다.

```
10-domains/10-cad2sim/flows/FLOW-CAD-001-dxf-to-simready.md
```
...
id: FLOW-CAD-001
domain: 10-cad2sim
title: DXF 단품 도면 -> SimReady USD
status: production          # draft | production | deprecated
graph: conductor/graphs/cad2sim.yaml
runs_total: 13
runs_passed: 12
last_verified: 2026-08-15
last_run: "[[RUN-20260815-1102-bracket]]"
---

## 1. 목적
2D DXF 단품 도면을 물리 시뮬레이션 가능한 SimReady 자산으로 만든다.

## 2. 입출력 계약
| | |
|---|---|
| 입력 | `.dxf` 1개, 폐곡선 외곽 + 선택적 구멍 |
| 필수 옵션 | 두께(m). 단위가 도면에 없으면 `DXF_UNITS` |
| 출력 | `<name>.usd` (crate), 질량·콜라이더·머티리얼 포함 |
| 실패 시 | 런 폴더 동결 + 원문 로그. 부분 산출물은 신뢰하지 않는다 |

## 3. 그래프
inspect -> convert -> usd-validate -> simready-baseline -> conform
       -> usd-revalidate -> simready-revalidate -> sim-check -> export

## 4. 검증 기준 (측정 가능한 것만)
- `run_pipeline.sh` exit = 0
- SimReady 프로파일 전 항목 PASS
- `sim_check.py` 120스텝: NaN 0, 관통 0
- 워터타이트: 모든 무향 엣지가 정확히 2회

## 5. 알려진 실패 모드          <-- 이 절이 진짜 자산이다
| 증상 | 원인 | 조치 | 최초 관측 |
|---|---|---|---|
| ManifoldChecker 경고인데 exit 1 | 구멍 루프를 두 번 뒤집음 | 벽 생성은 한 공식만 | [[RUN-20260712-...]] |
| `.usdc`에서 빈 `{}` 리포트 | simready-validate가 crate 미지원 | `--format auto` | [[RUN-20260714-...]] |
| FET004 실패 | Prop-* 프로파일이 강체 2개 요구 | `Prop-Static-Neutral` | [[RUN-20260716-...]] |

## 6. 최근 실행
```dataview
TABLE status, duration, file.link FROM "30-runs" WHERE flow = "FLOW-CAD-001"
SORT date DESC LIMIT 10
```

## 7. 변경 이력
- 2026-08-15 체이닝 허용 오차 기본값 조정
````

**5절이 시간이 갈수록 값이 커지는 유일한 부분이다.** 진단한 실패는 전부 여기 한 줄이 된다. 다음에 같은 증상이 나오면 에이전트가 재발명하지 않는다. **이게 1년 차보다 2년 차가 나아지는 유일한 메커니즘이다.**

플로우 문서를 만드는 시점은 **그 플로우가 두 번째로 성공했을 때**다. 한 번 성공은 우연일 수 있고, 미리 쓰면 실행되지 않는 계획서가 하나 더 늘 뿐이다.

### 18.4 런 리포트에는 그림이 있어야 한다

`0.31% 드리프트`라는 숫자만으로는 아무도 판단하지 못한다. **REPORT 단계는 시각 자료를 최소 1개 첨부해야 하며, 이건 권장이 아니라 규칙이다.**

| 런 종류 | 필수 첨부 | 생성 방법 | 비용 |
|---|---|---|---|
| 물리 실험 | 지표 시계열 **플롯 PNG** | CSV → matplotlib (`make_figures.py`) | CPU. 항상 가능 |
| 지오메트리 생성 | **와이어프레임/솔리드 캡처** | USD → 오프스크린 렌더 | 소량 GPU |
| 파라미터 스윕 | **비교 곡선 1장** | matplotlib | CPU |
| 동역학·유체 | **MP4 클립** (2–10초) | Isaac Sim 무비 캡처 | GPU. SIM 모드에서만 |
| 디지털 트윈 | **트랙 오버레이 프레임** | DeepStream 출력 | GPU |

**모드에 따라 갈린다.** HEADLESS 모드에서는 플롯만 만든다 — 렌더는 VRAM을 먹어 §4의 예산을 깨뜨린다. MP4가 필요하면 SIM 모드로 올려야 하고, 그건 Conductor가 PREFLIGHT에서 판단한다.

Obsidian은 이미지도 MP4도 그대로 렌더한다. 런 리포트는 이렇게 끝난다:

```markdown
## 결과
![[RUN-20260815-1102/energy-drift.png]]
![[RUN-20260815-1102/sim-clip.mp4]]

에너지 드리프트 0.31% (임계 2%). 관통 0건.
```

`30-runs/`가 무거워지므로 보존 정책을 둔다: **PNG는 영구, MP4는 90일 후 실패 런만 남기고 삭제.** 성공한 실험의 영상은 재현 가능하므로 보관 가치가 낮다.

### 18.5 `ENVIRONMENT.md` — 계획이 아니라 현재 상태

설계서의 하드웨어·경로·버전 절을 대체한다. **사람이 쓰지 않는다. Conductor가 주 1회 실측해서 덮어쓴다.**

```markdown
<!-- 자동 생성: 2026-08-17 03:00. 직접 수정하지 마라 -->
| 항목 | 값 | 확인 명령 |
|---|---|---|
| GPU / 드라이버 | RTX PRO 5000 Blackwell Laptop / 596.36 | `nvidia-smi` |
| Isaac Sim | 6.0.1 (Kit 110.1.2) | `isaac-sim.bat --version` |
| Python (ov) | 3.11.9, usd-exchange 설치됨 | `.venv-ov\Scripts\python -V` |
| 디스크 C: | 1,402 GB 여유 | `Get-PSDrive C` |
| 현재 모드 | HEADLESS | `conductor status` |
```

설계서가 "이렇게 만들 것"이라고 적힌 문서라면, 이건 "지금 이렇다"가 적힌 문서다. **에이전트는 후자만 믿는다.**

### 18.6 요약 — 무엇이 남고 무엇이 사라지나

```mermaid
flowchart LR
    D["07-agent-pc-design.md<br/>1,700줄<br/>구축용 소모품"]

    D --> A["90-meta/archive/<br/>Phase 0-11, 사전점검<br/>동결"]
    D --> B["도메인 CLAUDE.md x5<br/>루프 규약 · 게이트 · 격리<br/>상설"]
    D --> C["90-meta/architecture.md<br/>구성도 · 채널<br/>구조 변경 시만"]
    D --> E["ENVIRONMENT.md<br/>주 1회 자동 생성"]
    D --> F["20-facts/<br/>사실 카드<br/>만료 관리"]

    B --> G["flows/FLOW-*.md<br/>플로우당 1개<br/>실패 모드가 누적"]
    G --> H["30-runs/RUN-*.md<br/>그림·영상 포함<br/>불변 누적"]
    H -.->|"실패 진단이<br/>5절로"| G
...
**H → G 점선이 이 시스템의 학습 고리다.** 런에서 진단한 실패가 플로우 문서의 실패 모드 표로 올라가고, 다음 런이 그걸 읽는다. 이 화살표가 없으면 매번 같은 실수를 반복하는 자동화일 뿐이다.
