...
| `preflight-report.txt` | 이 PC의 실측 결과 (2026-08-10 + **08-11 개정**). **설계서보다 우선한다** |
| `AUTOMODE.md` | 자동 승인 모드 설정 |
...
| 드라이버 | 591.66 → **595.97 (08-11 갱신 완료)** ✅ |
| 디스크 | C: 1,795 GB 여유. **D: 없음 → `C:\agent`** |
| 절전 | Modern Standby(S0)만. S3·최대절전 없음<br>**유휴 5분이면 잠듦 — 현재 최우선 블로커** |
| 네트워크 | 13개 엔드포인트 전부 도달. 프록시 없음 |

---

## ① GPU 드라이버 갱신 — ✅ 2026-08-11 완료 (595.97)

**이 단계는 끝났다.** 595.97로 갱신되어 Isaac Sim 6.0.1 요구값을 정확히 충족한다.
아래는 재설치하거나 다른 PC에 깔 때를 위해 남겨둔다.

원래 상황은 **591.66**이었고 Isaac Sim 6.0.1이 요구하는 값은 **595.97**이다. 공식 요구사항
...
> **이제 남은 블로커는 드라이버가 아니라 절전 정책이다.** 이 PC는 지금도 유휴
> 5분이면 잠든다. 설계서 Phase 0-2가 최우선이며, 그 스킴에서는 `LIDACTION`이
> 숨김 처리되어 있어 set 후 반드시 읽어서 확인해야 한다.
